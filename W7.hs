...
pyramidi' :: Int -> Int -> String
pyramidi' 0 x = ""
pyramidi' n x = (pyramidi'' n) ++ (pyramidi''' x) ++ (pyramidi' (n-1) (x+1))
...
jokaToinen :: [a] -> [a]
jokaToinen [] = []
jokaToinen (x:d:xs) = x : (jokaToinen xs)
jokaToinen (x:xs) = x : xs
...
query :: Eq a => [a] -> a -> Bool
query [] n = False
query (x:xs) n = if x == n then True else query xs n
...
nousevat :: [Int] -> [[Int]]
nousevat [] = []
nousevat xs = a : nousevat (drop (length a) xs) where
  a = nousevat' xs

nousevat' :: [Int] -> [Int]
nousevat' [] = []
nousevat' [x] = [x]
nousevat' (x:d:xs) = if x < d then x : (nousevat' (d:xs)) else [x]
...
getName (Student name opnmr pst) = name

getNumber :: Student -> String
getNumber (Student name opnmr pst) = opnmr

getPoints :: Student -> Int
getPoints (Student name opnmr pst) = pst

addPoints :: Int -> Student -> Student
addPoints i (Student name opnmr pst) = if i >= 0 then Student name opnmr (pst+i) else Student name opnmr pst
...
treeHeight (Node3 l c r) = 1 + max (max(treeHeight r) (treeHeight c)) (treeHeight l)

treeSize :: Tree23 -> Int
treeSize Leaf = 0
treeSize (Node2 b c) = 1 + (treeSize b) + (treeSize c)
treeSize (Node3 b c d) = 1 + (treeSize b) + (treeSize c) + (treeSize d)
...
instance Eq MyString where
  MyString x == MyString s = x == s
  
instance Ord MyString where
  compare (MyString x) (MyString s) = if length x < length s
    then LT
    else if length x > length s 
      then GT
      else compare x s
...
safeEval (Constant s) = Just s
safeEval (Plus a s) = do
  x <- safeEval a
  y <- safeEval s
  Just (x+y)
safeEval (Div a s) = do
