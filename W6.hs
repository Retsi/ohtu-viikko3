...
pilko s = case break (== ' ') s of 
               (a, ' ':b) -> Just (a, b)
               otherwise -> Nothing


tarkastaNumero (a,b) = if any (isDigit) a == True then Nothing else if any isDigit b == True then Nothing else Just (a, b)


tarkastaIsotAlkukirjaimet (a, b) = if (head a == toUpper (head a)) == False then Nothing else if (head b == toUpper (head b)) == False then Nothing else Just (a, b) 
...
chainList [] = Just []
chainList (m:ms) = m ?> \x -> chainList ms ?> \cs -> Just (x :  cs)
...
sumPos xs = chainList (sumPos' xs) ?> (Just . sum)

sumPos' :: [Int] -> [Maybe Int]
sumPos' [] = []
sumPos' (x:xs) = if x < 0 then Nothing : (sumPos' xs) else Just x : (sumPos' xs)
...
myTake mi ml = do
  x <- mi
  xs <- ml
  if x > length xs then Nothing else Just (take x xs)
...
selectSum xs [] = Just 0
selectSum xs (c:is) = do
  x <- safeIndex xs c
  y <- selectSum xs is
  Just (x + y)

safeIndex :: [a] -> Int -> Maybe a
safeIndex xs x = if (length xs)-1 < x || (x < 0) then Nothing else Just (xs !! x)
...
binom n k
  | k == 0            = do msg ("B("++show n ++",0)")
                           return 1
  | (n == 0 && k > 0) = do msg ("B(0,"++show k ++")")
                           return 0
  | otherwise         = do x <- binom (n-1) (k-1)
                           y <- binom (n-1) k
                           msg ("B("++show n ++","++show k ++")")
                           return (x + y)
...
--    ==> (5,2)
--  runState (lengthAndCount False [True]) 1
--    ==> (1,1)

lengthAndCount :: Eq a => a -> [a] -> State Int Int
lengthAndCount x [] = do return 0
lengthAndCount x (c:xs) = do if (c == x) then modify (+1) else modify (+0)
                             g <- lengthAndCount x xs
                             return (1+g)
